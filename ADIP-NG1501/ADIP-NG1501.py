...
if __name__ == '__main__':
    ...
    if not os.path.exists(File_path_log_Run_Flag):
        with open(File_path_log_Run_Flag, "a", encoding='utf-8')as f:
            f.write("")
        File_paths_csv = [File_path_Search_Page_Info_CSV,
                          File_path_Shareholders_CSV]
        File_paths_txt = [File_path_Search_Page_Info_txt,
                          File_path_Shareholders_txt]
        File_path_index = [File_path_log_index_LetterE1,
                           File_path_log_index_LetterE2]
    ...
    try:
        for indexE1 in range(start_index_LetterE1, len(English_alphabet_list)):
            ...
            for indexE2 in range(start_index_LetterE2, len(English_alphabet_list)):
                ...
                # letter = '*A.G.M. ABDUL & CO. LTD (SEE RC 74590)'
                ...
                try:
                    ...
                    while searchpageRetry <= retry_attempts:
                        try:
                            ...
                            delay = retry_delay * (2 ** searchpageRetry)
                    ...
                    while statusRetry <= retry_attempts:
                        try:
                            Status = requests.post(Status_url, data=json.dumps(
                                Status_payload), headers=Status_headers, timeout=100)
                            break
                        except Exception as e:
                            if isinstance(e, ConnectionError):
                            ...
                            delay = retry_delay * (2 ** statusRetry)
                                        ...
                                        for item in data:
                                            if item['affiliatesPscInformation']:
                                                ...
                                                with open(File_path_Shareholders_txt, "a", encoding='utf-8') as fw:
                                                    fw.write("\t".join(map(str, Shareholders_Data))+"\n")
                            except Exception as e:
                                exception()
                            indi_ts_stop = time.time()
                            log_print(f'Added {company_name} {indi_ts_stop - indi_ts_start:.2f}')
                            # log_print(f"Added {indi_ts_stop - indi_ts_start:.2f}")
