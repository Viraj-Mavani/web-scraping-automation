                ...
                try:
                    ...
                    while searchpageRetry <= retry_attempts:
                        try:
                            ...
                            #     Search_Page_URL, data=payload, headers=headers, proxies=proxy)
                            Search_Page = requests.post(
                                Search_Page_URL, data=payload, headers=headers, timeout=100)
                            break
                        except Exception as e:
                            if isinstance(e, ConnectionError):
                                log_print(
                                    f"ConnectionError occurred in Search_Page for {letter}")
                            else:
                                log_print(
                                    f"Error occurred in Search_Page for {letter}")
                            delay = retry_delay * \
                                (2 ** searchpageRetry)
                            log_print(
                                f'Retrying in {delay} seconds...{searchpageRetry}')
                            time.sleep(delay)
                            searchpageRetry += 1
                            continue
                    else:
                    ...
                    while statusRetry <= retry_attempts:
                        try:
                            ...
                            if isinstance(e, ConnectionError):
                                log_print(
                                    f"ConnectionError occurred in status for {letter}")
                            else:
                                log_print(
                                    f"Error occurred in status for {letter}")
                            delay = retry_delay * \
                                (2 ** statusRetry)
                            log_print(
                                f'Retrying in {delay} seconds...{statusRetry}')
                            time.sleep(delay)
                            statusRetry += 1
                            continue
                    else:
                                    ...
                                    while shareholdersRetry <= retry_attempts:
                                        try:
                                            Shareholders_Info = requests.get(
                                                Shareholders_URL, headers=Share_headers, timeout=100)
                                            break
                                        except Exception as e:
                                            if isinstance(e, ConnectionError):
