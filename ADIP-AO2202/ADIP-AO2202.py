...
def individual_data(NIF, action_sp, indi_url):
	try:
		...
		if rowsT2:
			...
			indi_data[4] = folio_match.group(1).strip() if folio_match else ""
			try:
				Contro_de= folio_match.group(2).strip() if folio_match else ""
				if len(Contro_de)!=11:
					indi_data[5] = Contro_de 
				else:
					strong_tags=re.search(r'<strong>(.*?)</strong>\s*<strong>(.*?)</strong>',rowsT2content)
					if strong_tags:
						result = strong_tags.group(1) + " " + strong_tags.group(2)
						indi_data[5] = result
		...
		if searchBlock:
			for block in searchBlock:
				try:
					...
					# Action_URL = "https://gue.gov.ao/portal/publicacao/ver/3ZIppd"
